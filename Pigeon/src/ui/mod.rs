...
pub fn resolve_contact_or_args(
    ...
    if let Some(sel) = contact {
        ...
        let addr =
            to.ok_or_else(|| crate::error::Error::Config("--to or --contact required".into()))?;
        let hexpk = pubkey_hex.ok_or_else(|| {
            crate::error::Error::Config("--pubkey_hex or --contact required".into())
        })?;
        let remote_pk_bytes =
            hex::decode(hexpk).map_err(|e| crate::error::Error::Config(e.to_string()))?;
