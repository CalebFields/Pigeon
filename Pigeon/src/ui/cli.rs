    ...
    pub async fn execute(self) -> Result<(), crate::error::Error> {
        ...
        match self.command {
            ...
            Commands::Contacts { action } => {
                ...
                match action {
                    ContactsAction::Add { name, addr, pubkey_hex } => {
                        let c = store.add(&name, &addr, &pubkey_hex)
                            .map_err(crate::error::Error::Storage)?;
                    ...
                    ContactsAction::List => {
                        ...
                        for c in list {
                            println!("{}\t{}\t{}\t{}", c.id, c.name, c.addr, hex::encode(c.public_key));
                        }
                    }
                    ContactsAction::Remove { id } => {
                        let existed = store.remove(id).map_err(crate::error::Error::Storage)?;
                        if existed { println!("removed {}", id); } else { println!("not found: {}", id); }
                    }
                }
            }
            ...
            Commands::Inbox { action } => {
                match action {
                    InboxAction::List { queue, limit } => {
                        ...
                        if let Some(n) = limit { items.truncate(n); }
                    ...
                    InboxAction::Show { queue, id } => {
                        let queue_path = queue.unwrap_or_else(|| "queue_db".to_string());
                        let q = MessageQueue::new(&queue_path).map_err(crate::error::Error::Storage)?;
                        let uid = uuid::Uuid::parse_str(&id).map_err(|e| crate::error::Error::Serialization(e.to_string()))?;
            ...
            #[cfg(feature = "network")]
            Commands::SendLoop { queue, base_backoff, interval_ms } => {
                let cfg = crate::config::load();
                ...
                println!("Starting send loop (queue: {}, backoff: {}s, interval: {}ms)...", conf.queue_path, conf.base_backoff_secs, conf.interval_ms);
                crate::messaging::send_loop::run(conf).await?;
            }
            #[cfg(feature = "network")]
            Commands::SendNet { to, pubkey_hex, contact, message } => {
                use libp2p::{Multiaddr, swarm::SwarmEvent, Transport};
                let cfg = crate::config::load();
                ...
                let addr: Multiaddr = addr_str.parse().map_err(|e: libp2p::multiaddr::Error| crate::error::Error::Config(e.to_string()))?;
                ...
                while !done {
                    match swarm.select_next_some().await {
                        SwarmEvent::ConnectionEstablished { peer_id, .. } => {
                            ...
                            let ciphertext = sodiumoxide::crypto::box_::seal(&plaintext, &nonce, &remote_pk, &id.sodium_box_sk);
                            ...
                            let sig = sodiumoxide::crypto::sign::sign_detached(&to_sign, &id.sign_sk);
                            let env = crate::messaging::message::EnvelopeV1::new(0, 0, *nonce.as_ref(), ciphertext, sig.0.to_vec());
            ...
            #[cfg(feature = "network")]
            Commands::ListenNet { port, listen_addr, mdns } => {
                ...
                struct NodeBehaviour {
                    request_response: libp2p::request_response::Behaviour<crate::network::rr::PigeonCodec>,
                    mdns: libp2p::mdns::tokio::Behaviour,
                }
                let rr_cfg = libp2p::request_response::Config::default();
                let protocols: Vec<(String, libp2p::request_response::ProtocolSupport)> = vec![("/pigeon/1".to_string(), libp2p::request_response::ProtocolSupport::Full)];
                let rr_behaviour: libp2p::request_response::Behaviour<crate::network::rr::PigeonCodec> = libp2p::request_response::Behaviour::new(protocols, rr_cfg);
                let mdns_behaviour = libp2p::mdns::tokio::Behaviour::new(libp2p::mdns::Config::default(), local_key.public().to_peer_id())?;
                let behaviour = NodeBehaviour { request_response: rr_behaviour, mdns: mdns_behaviour };
                let peer_id = local_key.public().to_peer_id();
                let mut swarm = libp2p::Swarm::new(transport, behaviour, peer_id, libp2p::swarm::Config::with_tokio_executor());
                ...
                let addr_str = if let Some(cli_addr) = listen_addr {
                    ...
                    cfg_addr
                } else if let Some(p) = port { format!("/ip4/0.0.0.0/tcp/{}", p) } else { "/ip4/0.0.0.0/tcp/0".to_string() };
                let addr: Multiaddr = addr_str.parse().map_err(|e: libp2p::multiaddr::Error| crate::error::Error::Config(e.to_string()))?;
                    ...
                    match swarm.select_next_some().await {
                        ...
                        libp2p::swarm::SwarmEvent::Behaviour(ev) => {
                            match ev {
                                ...
                                NodeBehaviourEvent::RequestResponse(libp2p::request_response::Event::<Vec<u8>, Vec<u8>>::Message { message, .. }) => {
                                    match message {
                                        libp2p::request_response::Message::<Vec<u8>, Vec<u8>>::Request { request, channel, .. } => {
                                            ...
                                            if let Ok(env) = bincode::deserialize::<crate::messaging::message::EnvelopeV1>(&request) {
                                                ...
                                                let sig = match sodiumoxide::crypto::sign::Signature::from_slice(&env.signature) {
                                                    Some(s) => s,
                                                    None => { println!("received: <invalid signature>"); let _ = swarm.behaviour_mut().request_response.send_response(channel, b"NACK".to_vec()); return; }
                                                };
                                                // sender signing key currently not in envelope; accept self for demo
                                                let sender_sign_pk = &id.sign_pk;
                                                if sodiumoxide::crypto::sign::verify_detached(&sig, &to_verify, sender_sign_pk) {
                                                    // replay protection via nonce store
                                                    let db = sled::open(crate::config::load().data_dir.join("queue_db")).map_err(|e| crate::error::Error::Storage(e))?;
                                    ...
                                    if mdns || cfg.enable_mdns {
                                        match event {
                                            libp2p::mdns::Event::Discovered(list) => {
                                                for (peer, addr) in list { println!("mdns: discovered {peer} at {addr}"); }
                                            }
                                            libp2p::mdns::Event::Expired(list) => {
                                                for (peer, addr) in list { println!("mdns: expired {peer} at {addr}"); }
                                            }
                                        }
                                    }
                                }
