...
fn resolves_contact_by_name_and_id() {
    ...
    // Resolve by name
    let (addr1, pk1) =
        secure_p2p_msg::ui::resolve_contact_or_args(&data_dir, Some("alice"), None, None).unwrap();
    assert_eq!(addr1, c.addr);
    assert_eq!(pk1.0.as_slice(), c.public_key.as_slice());

    // Resolve by id
    let (addr2, pk2) =
        secure_p2p_msg::ui::resolve_contact_or_args(&data_dir, Some(&c.id.to_string()), None, None)
            .unwrap();
