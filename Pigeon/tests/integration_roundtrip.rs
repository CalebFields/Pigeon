...
async fn round_trip_and_drain_on_reconnect() {
    ...
    let _ = send_now(queue_b, &sk_a, &pk_b, 100, b"hello").await.unwrap();
    let _ = send_now(queue_b, &sk_a, &pk_b, 100, b"world").await.unwrap();
    ...
    let texts: Vec<String> = inbox.into_iter().map(|(_, v)| String::from_utf8_lossy(&v).to_string()).collect();
