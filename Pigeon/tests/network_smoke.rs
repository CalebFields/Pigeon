...
use libp2p::{identity, Multiaddr, swarm::SwarmEvent, Transport as _};
...
async fn two_swarms_connect_locally() {
    ...
    let transport1 = libp2p::tcp::tokio::Transport::new(libp2p::tcp::Config::default().nodelay(true))
    ...
    let mut swarm1 = libp2p::Swarm::new(transport1, behaviour1, kp1.public().to_peer_id(), libp2p::swarm::Config::with_tokio_executor());

    let transport2 = libp2p::tcp::tokio::Transport::new(libp2p::tcp::Config::default().nodelay(true))
        .upgrade(libp2p::core::upgrade::Version::V1)
        .authenticate(libp2p::noise::Config::new(&kp2).unwrap())
        .multiplex(libp2p::yamux::Config::default())
        .boxed();
    let behaviour2 = libp2p::ping::Behaviour::new(libp2p::ping::Config::new());
    let mut swarm2 = libp2p::Swarm::new(transport2, behaviour2, kp2.public().to_peer_id(), libp2p::swarm::Config::with_tokio_executor());
