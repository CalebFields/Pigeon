...
use libp2p::{identity, Multiaddr, swarm::{SwarmBuilder, SwarmEvent}};
...
async fn two_swarms_connect_locally() {
    ...
    let transport1 = libp2p::tcp::tokio::Transport::new(libp2p::tcp::Config::default().nodelay(true))
        ...
        .boxed();
    let behaviour1 = MessageProtocol::new();
    let mut swarm1 = SwarmBuilder::new(transport1, behaviour1, kp1.public().to_peer_id())
    ...
    let transport2 = libp2p::tcp::tokio::Transport::new(libp2p::tcp::Config::default().nodelay(true))
        ...
        .multiplex(libp2p::yamux::Config::default())
        .boxed();
    let behaviour2 = MessageProtocol::new();
    let mut swarm2 = SwarmBuilder::new(transport2, behaviour2, kp2.public().to_peer_id())
        .executor(Box::new(|fut| { tokio::spawn(fut); }))
        .build();
